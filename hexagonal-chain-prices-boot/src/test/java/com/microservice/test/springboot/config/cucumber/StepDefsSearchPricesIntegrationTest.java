...
@Slf4j
public class StepDefsSearchPricesIntegrationTest extends SpringIntegrationTest {

    private static final String URL = "http://localhost:8081/hexagonal-chain-prices/search";

    @When("the user calls with product_id {int} and application_date {string}")
    public void the_user_search_price_POST_200(int productId,String applicationDate) throws Throwable {
    ...
    public void the_user_receives_response_data(int statusCode,int brand,int product,int price,String start){
        final Integer currentStatusCode = latestResponse.getStatusCodeValue();

        if(statusCode==200){
            try {
                final Integer brandId = Objects.requireNonNull(latestResponse.getBody()).getBrandId();
